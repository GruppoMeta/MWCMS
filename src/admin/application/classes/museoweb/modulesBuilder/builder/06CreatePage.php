<?php
class museoweb_modulesBuilder_builder_06CreatePage extends museoweb_modulesBuilder_builder_AbstractCommand
{
	...
	function execute()
	{
		...
		$skinList .= '<div class="searchResults" tal:condition="php: !is_null(Component.records)">'.GLZ_COMPILER_NEWLINE2;
		$skinList .= '<h3 tal:content="structure Component/title"/>'.GLZ_COMPILER_NEWLINE2;
		$skinList .= '<span tal:omit-tag="" tal:condition="php: Component.records.count() > 0">'.GLZ_COMPILER_NEWLINE2;
		$skinList .= '<div tal:repeat="item Component/records" tal:attributes="class item/__cssClass__">'.GLZ_COMPILER_NEWLINE2;
		if ( empty( $firstImageId ) )
		{
			$skinList .= '<div class="around">'.GLZ_COMPILER_NEWLINE2;
		}
		else
		{
			if (!$firstImageRepeater) {
				$skinList .= '<div class="left" tal:condition="php: item.'.$firstImageId.'[\'mediaId\'] gt 0">'.GLZ_COMPILER_NEWLINE2;
				...
				$skinList .= '</div><div class="right">'.GLZ_COMPILER_NEWLINE2;
			} else {
				$skinList .= '<div class="left" tal:condition="php: count(item.'.$firstImageId.')>0">'.GLZ_COMPILER_NEWLINE2;
		...
		for ( $i = 0; $i < count( $fields ); $i++ )
		{
			if ( in_array( $fields[ $i ], $fieldListSearch ) )
			{
				...
				if ( !$first )
				{
					...
					$skinList .= '<h4><a href="" tal:attributes="href item/__url__; title item/'.$fields[ $i ].'" tal:content="structure item/'.$fields[ $i ].'"></a></h4>'.GLZ_COMPILER_NEWLINE2;
				}
				else
				{
		...
		$skinList .= '</div><div class="clear"></div>'.GLZ_COMPILER_NEWLINE2;
		$skinList .= '</div>'.GLZ_COMPILER_NEWLINE2;
		$skinList .= '</span>'.GLZ_COMPILER_NEWLINE2;
		$skinList .= '<span tal:omit-tag="" tal:condition="php: Component.records.count() == 0">'.GLZ_COMPILER_NEWLINE2;
		$skinList .= '<div class="around">'.GLZ_COMPILER_NEWLINE2;
		...
		$skinList .= '</div>'.GLZ_COMPILER_NEWLINE2;
		$skinList .= '</span>'.GLZ_COMPILER_NEWLINE2;
		$skinList .= '<div class="clear"></div>'.GLZ_COMPILER_NEWLINE2;
		$skinList .= '</div>'.GLZ_COMPILER_NEWLINE2;
		...
		$skinEntry .= '<div class="details">'.GLZ_COMPILER_NEWLINE2;
		$firstFieldName = '';
		for ( $i = 0; $i < count( $fields ); $i++ )
		{
			...
			else
			{
				...
				switch ( $types[ $i ] )
				{
					case 'IMAGEURL':
						...
						$img = '<span tal:omit-tag="" tal:condition="php: !empty( Component.'.$fields[ $i ].'[\'mediaUrl\'] )" tal:content="structure Component/'.$fields[ $i ].'/__html__" />'.GLZ_COMPILER_NEWLINE2;
						break;
					...
					case 'IMAGE':
						...
						$img = '<span tal:omit-tag="" tal:condition="php: Component.'.$fields[ $i ].'[\'mediaId\'] gt 0" tal:content="structure Component/'.$fields[ $i ].'/__html__" />'.GLZ_COMPILER_NEWLINE2;
						break;
