<?php
class museoweb_modulesBuilder_builder_Builder
{
	...
	function execute()
	{
		...
		{
			$c = org_glizy_ObjectFactory::createObject( 'museoweb.modulesBuilder.builder.' . $v, $this );
			$r = $c->execute();
			if ( !$r )
			{
				break;
	...
	{
		// crea le cartelle
		$sequence = array( 'd01DeleteFolders', 'd02CleanStartup', 'd03DeleteMenu', 'd04DeleteThesaurus' );
		foreach( $sequence as $v )
		{
			$c = org_glizy_ObjectFactory::createObject( 'museoweb.modulesBuilder.builder.' . $v, $this );
			$r = $c->execute();
