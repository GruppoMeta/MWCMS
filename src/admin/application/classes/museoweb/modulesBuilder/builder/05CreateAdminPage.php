<?php
class museoweb_modulesBuilder_builder_05CreateAdminPage extends museoweb_modulesBuilder_builder_AbstractCommand
{
	function execute()
	{
		...
		for ( $i = 0; $i < count( $fields ); $i++ ) {
			...
			switch ( $types[ $i ] )
			{
				...
				case 'HIDDEN':
					...
					break;
                case 'PICO':
                    $formFields .= '<pico:Thesaurus id="'.$fields[ $i ].'" label="'.$label.'" />'.GLZ_COMPILER_NEWLINE2;
                    break;
			}
		}
$output = <<<EOD
...
<glz:include
    ...
    <glz:template name="render_cell" value="$renderClass" />
